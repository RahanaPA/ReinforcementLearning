...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        # "if filled draw"
        if not any(space == 0 for space in curr_state):
            return True
    ...
    def step(self, curr_state, curr_action):
        ...
        isGameEnded, status = self.is_terminal(next_state)
        if isGameEnded == True:
            if status == 'Win':
                reward=10
            else:
            ...
            positions = random.choice(self.allowed_positions(next_state))
            values = random.choice(self.allowed_values(next_state)[1])
            ...
            if isGameEnded == True:
                ...
                reward=-1
        return next_state, reward, isGameEnded
